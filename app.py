...
def extract_contact_number_from_resume(text):
    ...
    # Improved regex pattern to find a potential contact number
    pattern = r"""
        (?:
...
def extract_name_from_resume(text):
    ...
    # Improved regex pattern to find potential names
    pattern = r"""
        \b
        ([A-Z][a-z]+)          # First name: capitalized first letter, followed by lowercase letters
        \s+
        ([A-Z][a-z]+(?:\s[A-Z][a-z]+)?) # Last name: one or two capitalized words
        \b
    """
